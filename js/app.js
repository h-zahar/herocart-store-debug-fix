...
// Function to Return an Array of Objects Containing Data 
const loadProducts = () => {
  ...
  // Call Function Passing the Array of Objects
  showProducts(data);
};
...
const showProducts = products => {
  ...
  // Loop through Every Product Object 
  for (const product of allProducts) {

    // Showing Image, Fixed Bug
    const image = product.image;
    const div = document.createElement("div");

    // div.classList.add("product"); [Commented as Not Necessary Yet!]

    // Creating a Card Element; Added Rating and Number of Ratings
    ...
    <img class="product-image" src=${image}></img>
      ...
      <button id="details-btn" class="" onclick="showDetails(${product.id})">Details</button></div>
      `;
    document.getElementById("all-products").appendChild(div);
  }
};

// Variable to Count Number of Products Added on Cart
let count = 0;

// Function to Pulse Count, Updating Price and Summing Up; Call on Add to Cart Button
const addToCart = (price) => {
  ...
  // Assigning and Showing Number of Products
  document.getElementById("total-Products").innerText = count;
};


// Function to Show Customized Modal Called on Details Button
const showDetails = id => {
  document.getElementById('show-info').style.display = 'block';
  document.getElementById('show-info').innerHTML = 'Loading';

  // Calling API Passing Product ID
  fetch(`https://fakestoreapi.com/products/${id}`)
  .then(res => res.json())
  .then(data => detailedInfo(data));
}
 
// Function to Show API Info through Modal
...
// Function to Convert Extracted String to Float
...
// Changed Integer to Float and Showed Two Floating Point on Cart
